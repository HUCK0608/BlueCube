Shader "Custom/Effect_Shader" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			float4 c = tex2D (_MainTex, float2(IN.uv_MainTex.x,IN.uv_MainTex.y+_Time.y*0.25 )) * _Color;
			...
			float4 final;			
			final = ( lerp( float4(c.rgb,1) , float4(0,0,0,0) , clamp( distanceDepth , 0.0 , 1.0 ) ) );
		...
		ENDCG
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				float4 c = tex2D(_MainTex2, float2(IN.uv_MainTex2.x, IN.uv_MainTex2.y - _Time.y)*0.3 + d.r);

				float _DT = lerp(1.5, 0, IN.uv_MainTex.y);
