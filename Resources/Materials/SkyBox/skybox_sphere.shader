...
#include "UnityCG.cginc"
	...
	float4 tex = tex2D(_Tex, tc);
		float4 tex2 = tex2D(_Tex4, float2(tc.x + _Time.x*0.1, tc.y));
		float4 tex3 = tex2D(_Tex2, tc.xy + float2(tex2.x*0.3, tex2.y*0.3));
		float4 tex4 = tex2D(_Tex3, float2(tc.x+_Time.x*0.05, tc.y*0.1));

		float4 final = lerp(tex, (tex3*1.5) * (tex4*0.8), tex3.a);
