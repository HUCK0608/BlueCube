...
public sealed class CheckGround_Intro : MonoBehaviour
{
    ...
    public bool Check()
    {
        ...
        {
            if (GameLibrary.Raycast2D(m_checkPoints[i].position, Vector2.down, out hit, Mathf.Infinity, layerMask))
            {
                m_distanceToGround = Vector2.Distance(m_checkPoints[i].position, hit.point);
