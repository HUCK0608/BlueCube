...
public enum E_PlayerState3D { Idle, Move, Attack, JumpUp, JumpDown, LadderInit, LadderIdle, LadderMove, PickInit, PickIdle, PickMove, PickJumpUp, PickJumpDown, PickEnd }
...
public sealed class PlayerController : MonoBehaviour
{
    ...
    private PlayerState m_currentState2D;
    ...
    {
        if(m_currentState2D != null)
        {
            m_currentState2D.enabled = false;
            m_currentState2D.EndState();
        }

        m_currentState2D = m_states2D[newState];

        m_currentState2D.InitState();
        m_currentState2D.enabled = true;
    }
}
